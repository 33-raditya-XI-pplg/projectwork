...
let HttpHeaders = {
    ...
    'Token': atob(__advanced.SECRET_TOKEN_API)
}
...
let _notif = function (selector, type, msg, scroll = true) {
    ...
    if (sl.length) {
        ...
        */
        ...
        log('the "' + selector + '" selector not found!')
        return false;
    }
}
...
function setUserSigned(value) {
    log('oke')
    return set_cookie(secretStorage(), btoa(value))
}

function userSigned() {
    return get_cookie(secretStorage())
}
...
function unsetArrayByKey(arr, key) {
    let arr_index = arr.indexOf(key)
    return arr.splice(arr_index, 1)
}
...
function segment_replace(index, text, glue = '') {
    return segment(index).replaceAll(text, glue)
}

const reverseObjectRecursion = (object = {}, keyToMatch = "", valueToMatch = "") => {
    if (isObject(object)) {
        ...
        for (let i = 0; i < entries.length; i += 1) {
            ...
            if (isObject(objectValue)) {
                const child = reverseObjectRecursion(objectValue, keyToMatch, valueToMatch);
...
function objParentReverse(obj, key, value, writeKey) {
    let reverseSaved = null;

    function saveData(data) {
        if (data[key] != value) {
            reverseSaved = {
                ...data,
                [writeKey]: reverseSaved
            }
            saveData(data[key])
        } else {
            reverseSaved = {
...
function findData(data, where){
    let result = []
    $.each(data, function(i,item){
        $.each(where, function(j,key){
           if(item[j] == key){
            result.push(item)
           }
        })
...
let fullScreenEl = $('[data-toggle="fullscreen"]')

fullScreenEl.each(function (i, el) {
...
function createEventAttr(event, attr, callback) {

    $(document).on(event, '[' + attr + ']', function (e) {
        callback(e)
    })
...
function generateEditorContent(selector){
    Simditor.locale = 'en-US';
    return new Simditor({
        textarea: $(selector),
        defaultImage: url('assets/img/logo.png'),
        imageButton: 'upload',
        upload: {
            url: generate_url_response('/editor/upload/'+randId(18)),
            params: {
                Authorization: "Bearer "+atob(get_cookie(secretStorage()))
            },
            fileKey: 'file',
            connectionCount: 3,
            leaveConfirm: 'Upload dalam proses, yakin ingin membatalkan?',
        },
        tabIndent: true,
        toolbar: true,
        toolbarFloat: true,
...
$('title').html(ENV.TITLE + ' | '+$('meta[name=title]').attr('content'))