                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton{{ $row->id_testimoni}}">
                                            ...
                                            <li>
                                                <form action="{{ route('testimoni.destroy', $row->id_testimoni) }}" method="POST" onsubmit="return confirm('Are you sure?');">
                                                    @csrf
                                                    @method('DELETE')
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('testimoni.update', $row->id_testimoni) }}" method="POST">
                    ...
                    <div class="mb-3">
                        <label for="tanggal" class="form-label">Tanggal</label>
                        <input type="date" name="tanggal" value="{{ $row->tanggal->format('Y-m-d') }}" class="form-control" placeholder="Tanggal" required>
                    </div>
                    <div class="mb-3">
                        <label for="rating" class="form-label">Rating</label>
                        <input type="number" name="rating" value="{{ $row->rating }}" class="form-control" placeholder="Rating" min="1" max="5" required>
                    </div>
                    <div class="mb-3">
                        <label for="isi_testimoni" class="form-label">Isi Testimoni</label>
                    ...
                    <div class="mb-3">
                        <label for="status_publikasi" class="form-label">Status Publikasi</label>
                        <select name="status_publikasi" class="form-select" required>
                            <option value="1" {{ $row->status_publikasi ? 'selected' : '' }}>Published</option>
                            <option value="0" {{ !$row->status_publikasi ? 'selected' : '' }}>Unpublished</option>
                        </select>
                    </div>
                    <div class="modal-footer justify-content-between mx-3">
                        <div>
