...
@section('content')
    ...
    <div class="bg-white rounded-4 px-3 py-3 mb-5 shadow-lg">
        <nav>
            <div class="nav nav-pills nav-justified" id="nav-tab" role="tablist">
                ...
                <button class="nav-link" id="nav-contact-tab" data-bs-toggle="tab" data-bs-target="#nav-live" type="button"
                    role="tab" aria-controls="nav-contact" aria-selected="false">Berlangsung</button>
                <button class="nav-link" id="nav-contact-tab" data-bs-toggle="tab" data-bs-target="#nav-end" type="button"
                        ...
                        <div class="container">
                           ...
                           <!-- Modal Create - Bagian Page Select (untuk form tambah) -->
                                ...
                                <div class="form-group mb-3">
                                    <label class="control-label mb-2">Upload Banner <span
                                    ...
                                    <div class="dropzone-wrapper">
                                        ...
                                        <input type="file" name="path_banner" class="dropzone" accept="image/*"
                                            required>
    ...
    {{-- edit --}}
    @foreach ($evt as $row)
        <div class="modal modal-lg fade" id="edit{{ $row->id_event }}" tabindex="-1" aria-labelledby="add"
            aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header bg-primary-gradient text-white">
                    ...
                    <div class="modal-body">
                        ...
                        {{-- form --}}
                        <form action="{{ route('event.update', $row->id_event) }}" method="POST"
                                ...
                                <div class="row">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select js-example-basic-single" name="page_id"
                                            aria-label="Default select example" data-placeholder="Pilih Page id" required>
                                            @foreach ($page as $set)
                                                <option value="{{ $set->id_page }}"
                                                    {{ $set->id_page == $row->page_id ? 'selected' : '' }}>
                                                    {{ $set->nama_page }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col">
                                        ...
                                        <div class="mb-3">
                                            <label for="nama_event" class="form-label">Nama Event</label>
                                            <input type="text" class="form-control" name="nama_event" id="nama_event"
                                                value="{{ $row->nama_event }}" required>
                                        </div>
                                        <div class="mb-3">
                                            <label for="tanggal_mulai" class="form-label">Tanggal Event</label>
                                            <input type="date" class="form-control" name="tgl_mulai"
                                                id="tanggal_mulai" placeholder="DD/MM/YYYY"
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select class="form-select js-example-basic-single" name="jenis_event_id"
                                                ...
                                                @foreach ($jenisEvt as $element)
                                                    <option value="{{ $element->id_jenis_event }}"
                                                        {{ $element->id_jenis_event == $row->jenis_event_id ? 'selected' : '' }}>
                                                        {{ $element->nama_jenis_event }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="mb-3">
                                            <label for="tempat_id" class="form-label">Tempat</label>
                                            <select class="form-select js-example-basic-single" name="tempat_id"
                                                aria-label="Default select example" data-placeholder="Pilih Kota"
                                                required>
                                                @foreach ($tempat as $set)
                                                    <option value="{{ $set->id_tempat }}"
                                                        {{ $set->id_tempat == $row->tempat_id ? 'selected' : '' }}>
                                                        {{ $set->nama_tempat }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col">
                                        ...
                                        <div class="mb-3 mt-4">
                                            ...
                                            <input type="date" class="form-control" id="tgl_berakhir"
                                                name="tgl_berakhir" placeholder="DD/MM/YYYY"
                                                value="{{ $row->tgl_berakhir }}" required>
                                        </div>
                                        <div class="mt-3">
                                            <label for="biaya_regis" class="form-label">Biaya</label>
                                            <div class="input-group mb-3">
                                                ...
                                                <input type="text" class="form-control currency"
                                                    aria-label="Dollar amount (with dot and two decimal places)"
                                                    name="biaya_regis" value="{{ $row->biaya_regis }}" required>
                                            </div>
                                        </div>
                                        <div class="mb-3">
                                            <label for="visibilitas" class="form-label">Visibilitas</label>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="dropzone-wrapper">
                                            ...
                                            <input type="file" class="dropzone" name="path_banner"
                                                id="path_banner_{{ $row->id_event }}" accept="image/*">
                                            <div id="image_preview_" class="mt-3 d-flex justify-content-center">
                                                @if ($row->path_banner)
                                                    <img id="preview_image_edit_{{ $row->id_event }}"
                                                        src="{{ asset($row->path_banner) }}" alt="image preview"
                                                        style="max-width: 100%; max-height: 100%; object-fit: contain;">
                                                @else
                                                    <img id="preview_image_edit_{{ $row->id_event }}" src=""
                                        ...
                                        <div class="mt-2">
                                            <small style="color: red;">Format harus berupa: .jpg, .jpeg, .png, .bmp dan
                    ...
                    <div class="modal-footer justify-content-between mx-3">
                        <div class="form-check form-switch">
                            ...
                            <input class="form-check-input" type="checkbox" role="switch" id="status"
                                ...
                                {{ in_array($row->status, ['Selesai', 'Berlangsung', 'Publish']) ? 'checked' : '' }}>
